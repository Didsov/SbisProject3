...
### Управление отправкой с рабочего ПК

**Текущую схему менять не нужно.**

`sender.py` запускается на рабочем Windows-ПК и подключается к SMTP-серверу на VPS
по защищённому submission-порту `587`.
...
То есть VPS не обязан запускать основной `sender.py`.
Рабочий ПК продолжает управлять очередью, выбирать клиентов, формировать письма и
решать, когда выполнять отправку.

Это удобно, потому что `data/project.db` остаётся локальной рабочей БД, а VPS
используется как:

- SMTP relay;
- DKIM signer;
- публичный HTTPS endpoint для BIMI;
- будущий tracking endpoint.

При развёртывании tracking-сервиса на VPS эту схему сохраняем.
...
Поскольку `project.db` находится на Windows-ПК, публичный VPS **не должен**
пытаться напрямую работать с `D:\GIT\ProjectSbis\data\project.db`.

Планируется отдельное серверное хранилище входящих tracking-событий, после чего
локальный ProjectSbis будет синхронизировать их в `mail_events`.
...
- реальный sender запускается на Windows-ПК и использует VPS как SMTP relay через 587/STARTTLS;
...
- `project.db` остаётся на рабочем ПК и не должен копироваться на VPS;
- серверная tracking-БД должна быть отдельной;
...
- следующий этап: запушить текущий код в GitHub и развернуть tracking-компонент на VPS.
